...
and prints each of them to the console, along with the number of goals that were scored in 
...
// OU const [players1, players2] = game.players;
...
const printGoals = function (...playerName) {
  for (player of playerName) {
    ...
    if (game.scored.includes(player)) {
      goals = game.scored.filter((x) => x === player).length;
    }
    console.log(player + " has put " + goals + " goals !");
  }
};
printGoals("Davies", "Muller", "Lewandowski", "Kimmich", "Hummels");
// 7.
// console.log(Math.max(team1, team2));
