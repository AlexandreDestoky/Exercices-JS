...
para.textContent = "Answer poll";
document.body.append(para);

let btn = document.querySelector("button");

const poll = {
  question: "Quel est votre language de programmation préféré ?",
...
const registerNewAnswer = function () {
  let options = "";
  for (let option of this.options) options += option + "\n";
  // console.log(this.question);
  let choix = prompt(`${this.question}\n${options}`);
  choix = Number(choix);
  (choix >= 0 && choix < this.options.length) ? this.answers[choix]++ : alert("Mauvais numéro");
  displayResults.call(poll, "array");
};

btn.addEventListener("click", registerNewAnswer.bind(poll));
